...
PLCAE_PATH = os.path.join(DATA_PATH, 'Place')
...
class Trainer():
    def __init__(self,) -> None:
        ...
        self.dynamic_mixup = False
        self.mixup_alpha_min = 0.1
        self.mixup_alpha_max = 0.1
        self.mixup_difficulty_power = 1.0
        self.dynamic_use_accuracy = False
        self.class_acc_path = ''
        ...
        self.detach_aug = 'no'
        self.aug_data = 'no'
        self.clswarm_scale = 60

        self.ending_block = 100

        self.group_file = 'code_group_v3'
        self.group_index = 0
        self.reg_weight = 0.1
        self.reg_clamp = 47.0
        self.ood_loss_weight = 0.1
    ...
    def finetune(self):
        ...
        dynamic_mixup_flag = '--dynamic_mixup' if self.dynamic_mixup else ''

        print(self.gran_path)
        # sys.exit()
        os.system(f'''python -m torch.distributed.launch \
                    --nproc_per_node={nodes} \
                    --master_port {self.master_port} \
                    ...
                    --warmup_epochs {self.warmup_epochs} \
                    --blr {self.blr} \
                    --gran_path {self.gran_path} \
                    --mask_type {self.mask_type} \
                    --detach_aug {self.detach_aug} \
                    --aug_data {self.aug_data} \
                    --clswarm_scale {self.clswarm_scale} \
                    ...
                    --mixup {self.mixup} \
                    --cutmix {self.cutmix} \
                    --mixup_alpha_min {self.mixup_alpha_min} \
                    --mixup_alpha_max {self.mixup_alpha_max} \
                    --mixup_difficulty_power {self.mixup_difficulty_power} \
                    --class_acc_path {self.class_acc_path} \
                    {"--dynamic_use_accuracy" if self.dynamic_use_accuracy else ""} \
                    --data_path {data_path} \
                    --dataset {self.dataset} \
                    --imbf {self.imbf} \
                    --nb_classes {self.nb_classes} \
                    --num_workers {self.num_workers} \
                    --clswarm {self.clswarm} \
                    --ending_block {self.ending_block} \
                    --text_classifier {self.text_classifier} \
                    --group_file {self.group_file} \
                    --group_index {self.group_index} \
                    --reg_weight {self.reg_weight} \
                    --reg_clamp {self.reg_clamp} \
                    --ood_loss_weight {self.ood_loss_weight} \
                    --prit {self.prit} \
                    {attn_only} {cls_type} {dynamic_mixup_flag} \
                    --dist_eval
                ''')
